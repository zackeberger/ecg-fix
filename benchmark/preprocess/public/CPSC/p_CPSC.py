...
def main(config=None):
    config = config or load_config()
     # safer multiprocessing start (recommended with torch)
    try:
        mp.set_start_method("spawn", force=True)
    except RuntimeError:
        pass

    base_dir = dataset_root(config, "CPSC", "challenge-2020", "1.0.2", "training", "cpsc_2018")
    ...
    # ---- multiprocessing ----
    n_workers = min(os.cpu_count() or 1, 40)
