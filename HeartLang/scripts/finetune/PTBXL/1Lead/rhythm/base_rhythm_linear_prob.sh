...
for ratio in "${split_ratios[@]}"; do
    ...
    for method in "${sampling_methods[@]}"; do
        ...
        torchrun --nnodes=1 --master_port=$PORT --nproc_per_node=1 run_class_finetuning.py \
            ...
            --output_dir checkpoints/finetune/ptbxl/finetune_rhythm_base_linear_${ratio}_${method}/ \
            --log_dir log/finetune/finetune_rhythm_base_linear_${ratio}_${method} \
...
for ratio in "${split_ratios[@]}"; do
    ...
    for method in "${sampling_methods[@]}"; do
        ...
        torchrun --nnodes=1 --master_port=$PORT --nproc_per_node=1 run_class_finetuning.py \
            --dataset_dir datasets/ecg_datasets/PTBXL_QRS/rhythm \
            --output_dir checkpoints/finetune/ptbxl/finetune_rhythm_base_linear_${ratio}_${method}/ \
            --log_dir log/finetune_test/finetune_rhythm_base_linear_${ratio}_${method} \
            --model HeartLang_finetune_base \
            --eval \
            --trainable linear \
            --split_ratio $ratio \
