...
class EchoNextNPYDataset(Dataset):
    """
    Unified EchoNext dataset saved as:
      - echonext_ecg.npy
      - echonext_labels.npy
      - echonext_metadata.csv  (contains split column)
      - echonext_valid_idx.npy
      - echonext_class_to_index.json
    """
    ...
    def __init__(
        ...
        meta_path = meta_path or os.path.join(config["processed_dir"], "echonext_metadata_final.csv")
        ...
        # class map
        class_map_path = os.path.join(root_dir, "echonext_class_to_index.json")
        with open(class_map_path, "r") as f:
            self.class_to_index: Dict[str, int] = json.load(f)
        self.vocab = list(self.class_to_index.keys())

        # memmaps
        self.ecg = np.load(os.path.join(root_dir, "echonext_ecg.npy"), mmap_mode="r")
        self.labels = np.load(os.path.join(root_dir, "echonext_labels.npy"), mmap_mode="r")
