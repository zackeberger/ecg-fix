...
REMOVE_CODES = {"ILBBB", "PMI", "WPW", "SEHYP", "AFLT", "BIGU", "PSVT", "SVTAC", "TRIGU", "HVOLT", "PRC(S)", "STE_", "TAB_"}
# ----------------------------
# helpers
# ----------------------------
...
def build_vocab_and_mapping(scp_df: pd.DataFrame, label_type: str):
    ...
    if label_type in {"form", "rhythm"}:
        ...
        category_scp.difference_update(REMOVE_CODES)
    ...
    if label_type in {"diagnostic_class", "diagnostic_subclass"}:
        ...
        classes.difference_update(REMOVE_CODES)
...
def export_ptbxl_c_ecg_500hz(dataset_root: str, out_dir: str, n_workers: int = 8):
    """
    Exports ECG once:
    ...
    out_ecg = os.path.join(out_dir, "ptbxl_c_ecg_500hz.npy")
    out_valid = os.path.join(out_dir, "ptbxl_c_valid_idx.npy")
    out_base_meta = os.path.join(out_dir, "ptbxl_c_base_metadata.csv")

    print(f"Exporting PTB-XL C ECG 500Hz | N={N} | workers={n_workers}")
...
def export_ptbxl_c_labels_and_metadata(dataset_root: str, out_dir: str, label_type: str):
    """
    Creates per-label-type:
      - ptbxl_c_{label_type}_labels.npy   (N, C) uint8
      - ptbxl_c_{label_type}_metadata.csv (includes split + label_ columns)
      - ptbxl_c_{label_type}_class_to_index.json
    Uses base metadata + valid_idx (hard mask).
    """
    assert label_type in {"form", "rhythm","diagnostic_subclass"}

    # load base/meta
    base_meta = pd.read_csv(os.path.join(out_dir, "ptbxl_c_base_metadata.csv"))
    valid_idx = np.load(os.path.join(out_dir, "ptbxl_c_valid_idx.npy")).astype(np.int64)

    # load raw ptbxl_c df + scp
    ...
    out_lbl = os.path.join(out_dir, f"ptbxl_c_{label_type}_labels.npy")
    out_meta = os.path.join(out_dir, f"ptbxl_c_{label_type}_metadata.csv")
    out_map = os.path.join(out_dir, f"ptbxl_c_{label_type}_class_to_index.json")
...
def main(config):
    ...
    for lt in ["form", "rhythm", "diagnostic_subclass"]:
        ...
        export_ptbxl_c_labels_and_metadata(dataset_root, out_dir, lt)
