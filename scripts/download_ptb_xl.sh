...
PTBXL_VERSION="1.0.3"
PTBXL_URL="https://physionet.org/files/ptb-xl/${PTBXL_VERSION}/"
MAX_REPAIR_ATTEMPTS="${MAX_REPAIR_ATTEMPTS:-10}"

usage() {
  cat <<'USAGE'
Download PTB-XL using SHA256SUMS.txt as both a file manifest and checksum verifier.
...
check_sha256_tool() {
  if command -v sha256sum >/dev/null 2>&1; then
    ...
    echo "sha256sum or shasum is required to verify PTB-XL." >&2
    exit 1
  fi
}
...
download_manifest() {
  ...
  echo "Downloading PTB-XL manifest/checksum file: SHA256SUMS.txt"
  wget -N -c -P "$PTBXL_DIR" "${PTBXL_URL}SHA256SUMS.txt"
}

list_expected_files() {
  sed -E 's/^[a-fA-F0-9]{64}[[:space:]]+\*?//' "$PTBXL_DIR/SHA256SUMS.txt" |
    sed '/^[[:space:]]*$/d'
}
...
download_files_from_list() {
  ...
  while IFS= read -r relpath; do
    ...
    local local_dir
    local_dir="$PTBXL_DIR/$(dirname "$relpath")"
    mkdir -p "$local_dir"

    echo "Downloading: $relpath"
    wget -c -P "$local_dir" "${PTBXL_URL}${relpath}"
  done < "$list_file"
}
...
verify_checksums_and_write_bad_files() {
  ...
  # Parse checksum failures from sha256sum/shasum output.
  awk '
    /: FAILED$/ {
      sub(/: FAILED$/, "")
      print
...
PTBXL_DIR="${PHYSIONET_DIR}/ptb-xl/${PTBXL_VERSION}"
MISSING_LIST="$(mktemp)"
BAD_LIST="$(mktemp)"
VERIFY_LOG="$(mktemp)"
...
while [[ "$attempt" -le "$MAX_REPAIR_ATTEMPTS" ]]; do
  echo
  echo "PTB-XL repair attempt $attempt of $MAX_REPAIR_ATTEMPTS"
  ...
  if verify_checksums_and_write_bad_files; then
    echo
    echo "Done. PTB-XL is complete and all checksums passed."
    echo "PTB-XL location: $PTBXL_DIR"
    exit 0
  fi
...
echo
echo "PTB-XL still did not pass after $MAX_REPAIR_ATTEMPTS repair attempts." >&2
