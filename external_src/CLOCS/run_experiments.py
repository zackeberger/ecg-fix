...
dataset_list = ['mimiciv', 'physionet','physionet2017','cardiology','ptb','fetal','physionet2016','physionet2020','chapman','chapman_pvc']#,'cipa']
batch_size_list = [256, 256, 256, 16, 64, 64, 256, 256, 256, 256]#, 512]
lr_list = [1e-4, 1e-4, 1e-4, 1e-4, 5e-5, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4]#, 1e-4]
nleads = 12 # 12 | 4
if nleads == 12:
    leads_list = ["['I','II','III','aVR','aVL','aVF','V1','V2','V3','V4','V5','V6']",None,None,None,'i','Abdomen 1','i',"['I', 'II', 'III', 'aVL', 'aVR', 'aVF', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6']","['I', 'II', 'III', 'aVR', 'aVL', 'aVF', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6']","['I', 'II', 'III', 'aVR', 'aVL', 'aVF', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6']"] #'II' for one lead, ['II','V1',etc.] for more leads
elif nleads == 4:
            ...
            for downstream_dataset in downstream_dataset_list: #dataset used for pretraining
                for second_dataset in second_dataset_list: #dataset used for evaluation down the line
                    for labelled_fraction in labelled_fraction_list:
                        ...
                        max_epochs = 50 #hard stop for training
                        max_seed = 1
                        lr = 1e-5
                        temp = .1
                        config = {"temp":temp,"lr":lr, "batch_size": batch_size, "embedding_size":embedding_dim}
                        wandb.init( project="CLOCS", config=config)
                        seeds = np.arange(max_seed)
                        for seed in seeds:
                          #  save_path_dir, seed = make_saving_directory_contrastive(downstream_task, downstream_dataset,trial_to_load,trial_to_run,seed,max_seed,downstream_task,embedding_dim, held_out_lr, temp)
                      ...
                      #     load_path_dir, save_path_dir = obtain_load_path_dir(phases,save_path_dir,trial_to_run,second_dataset,labelled_fraction,leads,max_seed,downstream_task)
                           ...
                           #     continue
                            load_path_dir =""
                            save_path_dir= make_save_dir("/storage/doprakah/CLOCS/runs", config)
                            
                            classification = determine_classification_setting(second_dataset,trial_to_run)
                            train_model(temp, basepath_to_data,cnn_network_contrastive,second_cnn_network,classification,load_path_dir,save_path_dir,seed,batch_size,lr,fraction,modalities,leads,saved_weights,phases,original_downstream_dataset,downstream_task,class_pair,input_perturbed,perturbation,trial_to_load=trial_to_load,trial_to_run=trial_to_run,nencoders=nencoders,embedding_dim=embedding_dim,nviews=nviews,labelled_fraction=labelled_fraction,num_epochs=max_epochs)

import os

def make_save_dir(base_dir, config):
    parts = [f"{k}-{config[k]}" for k in sorted(config.keys())]
    folder = "_".join(parts)
    save_path = os.path.join(base_dir, folder)
    os.makedirs(save_path, exist_ok=True)
    return save_path

basepath_to_data = '/storage/shared/mimic-iv'
phases = ['train','val']#['test'] #['train','val'] #['test']
trial_to_load_list = ['CMSC'] #for loading pretrained weights
...
downstream_dataset_list = ['mimiciv']#,'physionet2020'] #dataset for pretraininng # 'chapman' | 'physionet2020'
