...
def train_model(temp, basepath_to_data,cnn_network_contrastive,second_cnn_network,classification,load_path_dir,save_path_dir,seed,batch_size,held_out_lr,fraction,modalities,leads,saved_weights,phases,downstream_dataset,downstream_task,class_pair,input_perturbed,perturbation,trial_to_load=None,trial_to_run=None,nencoders=1,embedding_dim=256,nviews=1,labelled_fraction=1,num_epochs=250):
    ...
    stop_counter = 0
    patience = 50 #for early stopping criterion
    epoch_count = 0
    ...
    while stop_counter <= patience and epoch_count < num_epochs:
        ...
        for phase,inference in zip(phases,inferences):
            ...
            if trial_to_run in ['Linear','Fine-Tuning','Random']:
                ...
                results_dictionary, outputs_list, labels_list, modality_list, indices_list, task_names_list, pids_list = one_epoch_contrastive(temp, weighted_sampling,phase,inference,dataloader,model,optimizer,device,trial=trial_to_run,epoch_count=epoch_count,save_path_dir=save_path_dir)
            ...
            if inference == False:
                ...
                if 'train1' in phase:
                    wandb.log({"avg_epoch_train_loss": epoch_loss})
                elif 'val' in phase:
                    wandb.log({"avg_epoch_val_loss": epoch_loss})
                ...
                if phase == 'val' and epoch_loss < best_loss or phase == 'test' and epoch_loss < best_loss:
                    ...
                    if 'train1' in phases:
                        save_config_weights(save_path_dir,best_model_wts,"best_weights",phases,trial_to_run,downstream_dataset)
                ...
                save_config_weights(save_path_dir,latest_weights,"latest_weights",phases,trial_to_run,downstream_dataset)
                
                metrics_dict = track_metrics(metrics_dict,results_dictionary,phase,epoch_count)  
             #   if 'train1' in phases:
             #       save_metrics(save_path_dir,"train_val",metrics_dict)  
           #     elif 'val' in phases:
             #       save_metrics(save_path_dir,"train_val",metrics_dict)               
