...
def temporal_12lead_contrastive_loss(z, temperature=0.1):
    """
    z: (B, E, 2)
    """
    assert z.shape[2] == 2, f"Expected 2 views, got {z.shape[2]}"
...
def contrastive_single(temp, phase,inference,dataloaders,model,optimizer,device,weighted_sampling,epoch_count=None,new_task_epochs=None,trial=None,save_path_dir=None): #b/c it is single, models_list contains one model only
    ...
    #    print(inputs.shape) # (1,2500,12,2)
        ...
        """ Calculate Metrics """
        running_loss += float(loss.detach()) * inputs.shape[0]
   #     print("vhjvghj")
...
def one_epoch_contrastive(temp, weighted_sampling,phase,inference,dataloader,model,optimizer,device,bptt_steps=0,epoch_count=None,new_task_epochs=None,trial=None,save_path_dir=None):
    epoch_loss, outputs_list, labels_list, modality_list, indices_list, task_names_list, pids_list = contrastive_single(temp, phase,inference,dataloader,model,optimizer,device,weighted_sampling,epoch_count=epoch_count,new_task_epochs=new_task_epochs,trial=trial,save_path_dir=save_path_dir)
